...
namespace eae6320
{
	namespace Graphics
	{
		...
		bool Mesh::ReadSquareValues(const char* const path)
		{
			...
			is.close();
			m_verticesCount = *reinterpret_cast<uint32_t*>(buffer);
			m_vertexData = reinterpret_cast<sVertex*>(buffer + sizeof(uint32_t));
			m_indicesCount = *reinterpret_cast<uint32_t*>(buffer + sizeof(uint32_t) + (sizeof(sVertex)*m_verticesCount));
			m_indexData = reinterpret_cast<uint32_t*>(buffer + sizeof(uint32_t) + (sizeof(sVertex)*m_verticesCount) + sizeof(uint32_t));
		...
		vertexData = new Mesh::sVertex[verticesCount];
		...
		lua_gettable(&io_luaState, -2);
		vertexData[i-1].x =(float)lua_tointeger(&io_luaState, -1);
		lua_pop(&io_luaState, 1);
		lua_pushinteger(&io_luaState, 2);
		lua_gettable(&io_luaState, -2);
		vertexData[i-1].y = (float)lua_tointeger(&io_luaState, -1);
		lua_pop(&io_luaState, 1);
		...
		lua_pushinteger(&io_luaState, 1);
		lua_gettable(&io_luaState, -2);
		vertexData[i-1].r =255 * (int)lua_tointeger(&io_luaState, -1);
		lua_pop(&io_luaState, 1);
		lua_pushinteger(&io_luaState, 2);
		lua_gettable(&io_luaState, -2);
		vertexData[i-1].g = 255 * (int)lua_tointeger(&io_luaState, -1);
		lua_pop(&io_luaState, 1);
		lua_pushinteger(&io_luaState, 3);
		lua_gettable(&io_luaState, -2);
		vertexData[i-1].b = 255 * (int)lua_tointeger(&io_luaState, -1);
		lua_pop(&io_luaState, 1);
		lua_pushinteger(&io_luaState, 4);
		lua_gettable(&io_luaState, -2);
		vertexData[i-1].a = 255 * (int)lua_tointeger(&io_luaState, -1);
		lua_pop(&io_luaState, 1);
		...
		lua_pushinteger(&io_luaState, 1);
		lua_gettable(&io_luaState, -2);
		indexData[(i- 1) * 3] = (int)lua_tointeger(&io_luaState, -1);
		lua_pop(&io_luaState, 1);
		lua_pushinteger(&io_luaState, 2);
		lua_gettable(&io_luaState, -2);
		indexData[(i - 1) * 3 + 2] = (int)lua_tointeger(&io_luaState, -1);
		lua_pop(&io_luaState, 1);
		lua_pushinteger(&io_luaState, 3);
		lua_gettable(&io_luaState, -2);
		indexData[(i - 1) * 3 + 1] = (int)lua_tointeger(&io_luaState, -1);
		lua_pop(&io_luaState, 1);
